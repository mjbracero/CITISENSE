...
let cachedModeratorProfile = DEFAULT_PROFILE_INFO;
...
function buildModeratorProfileInfo(currentUser, databaseProfile = null) {
  ...
  return {
    name:
      ...
      metadata.username ||
      authEmail?.split("@")[0] ||
      cachedModeratorProfile.name,

    email:
      databaseProfile?.email ||
      authEmail ||
      cachedModeratorProfile.email ||
    ...
    contact:
      ...
      cachedModeratorProfile.contact ||
    ...
    department:
      ...
      cachedModeratorProfile.department ||
      DEFAULT_DEPARTMENT,
    ...
    avatarUrl:
      ...
      metadata.avatar_url ||
      cachedModeratorProfile.avatarUrl ||
      null,
  };
}
...
export default function ModeratorProfile() {
  ...
  const [profileInfo, setProfileInfo] = useState(cachedModeratorProfile);
  ...
  const loadUser = useCallback(async () => {
    try {
      ...
      if (authEmail) {
        ...
        if (profileError) {
          console.log("Moderator profile database load error:", profileError);
        }

        databaseProfile = profileData || null;
      }

      const nextProfileInfo = buildModeratorProfileInfo(
        currentUser,
        databaseProfile
      );

      cachedModeratorProfile = nextProfileInfo;
      setProfileInfo(nextProfileInfo);
      setUser(currentUser);
    } catch (error) {
      console.log("Load moderator profile error:", error);
      // Keep cached profile to avoid UI jumping.
    }
  }, []);

  const checkLocationPermission = useCallback(async () => {
    if (hasCheckedLocationPermission) {
    ...
    try {
      ...
      cachedLocationEnabled = isGranted;
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const nextProfileInfo = buildModeratorProfileInfo(user, payload.new);
  ...
  const openEditProfile = () => {
    setFullNameDraft(displayName === "Moderator User" ? "" : displayName);
    setEmailDraft(displayEmail === "moderator@example.com" ? "" : displayEmail);
      ...
      try {
        ...
        if (permission.status === "granted") {
          ...
          Alert.alert(
            ...
            "CitiSense can now use your location for map routing, pinned complaint navigation, and moderator location tracking."
          );
        } else {
  ...
  const saveProfileChanges = async () => {
    ...
    try {
      ...
      if (emailChanged) {
        ...
        Alert.alert("Profile Updated", "Your moderator information was saved.");
      }
    } catch {
  ...
  const handlePrivacy = () => {
    Alert.alert(
      "Privacy & Security",
      "CitiSense protects moderator accounts by allowing only authorized department moderators to access assigned complaint records.\n\n" +
        ...
        "Your location permission may be used for map routing, pinned complaint navigation, and viewing the moderator’s current location while handling assigned complaints.\n\n" +
    ...
    Alert.alert("Log Out", "Are you sure you want to log out?", [
      ...
      },
      {
        ...
        onPress: async () => {
          ...
          cachedModeratorProfile = DEFAULT_PROFILE_INFO;
          cachedLocationEnabled = false;
        ...
        >
          <View style={styles.profileHeroCard}>
            ...
            <View style={styles.profileMainInfo}>
              ...
              <Text style={styles.profileRole}>Department Moderator</Text>
          ...
          <View style={styles.sectionBlock}>
            ...
            <View style={styles.settingsCard}>
              <ActionRow
                ...
                subtitle="Update your moderator password."
        ...
        <View style={styles.bottomNav}>
          {bottomTabs.map((tab) => {
            const isActive =
              ...
              (tab.label === "Profile" &&
                pathname?.includes("moderatorProfile"));
