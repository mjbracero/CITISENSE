...
function createEditableMapHtml({ latitude, longitude, styleUrl }) {
...
function createPreviewMapHtml({ latitude, longitude, styleUrl }) {
  const lat = Number(latitude);
  const lng = Number(longitude);
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    return `
      <!DOCTYPE html>
      <html>
        <body style="margin:0;height:100%;display:flex;align-items:center;justify-content:center;background:#f3f5f3;color:#6f776f;font-family:Arial;font-size:12px;">
...
const bottomTabs = [
  {
    ...
    route: "/(tabs)/citizenDashboard",
    activePath: "citizenDashboard",
    flex: 0.85,
  },
  {
    label: "Submit",
    ...
    route: "/(tabs)/citizenSubmit",
    activePath: "citizenSubmit",
    flex: 0.9,
  },
  {
    label: "My Complaints",
    ...
    route: "/(tabs)/citizenComplaints",
    activePath: "citizenComplaints",
    flex: 1.45,
  },
  {
    label: "Profile",
    ...
    route: "/(tabs)/citizenProfile",
    activePath: "citizenProfile",
    flex: 0.85,
  },
];
...
function buildAddress(place) {
  if (!place) return "Barangay Sambag, Bogo City, Cebu";
  ...
  return parts.length > 0
    ? parts.join(", ")
    : "Barangay Sambag, Bogo City, Cebu";
}
...
async function normalizePhotoForUpload(photo) {
  ...
  const fileInfo = await FileSystem.getInfoAsync(manipulatedPhoto.uri, {
    size: true,
  });
  ...
  const base64Photo = await FileSystem.readAsStringAsync(manipulatedPhoto.uri, {
    encoding: FileSystem.EncodingType.Base64,
  });

  return {
    arrayBuffer: decode(base64Photo),
...
export default function CitizenSubmit() {
  ...
  const [mapEditorSeed, setMapEditorSeed] = useState(null);
  const [selectedLocation, setSelectedLocation] = useState(DEFAULT_REGION);
  const [temporaryLocation, setTemporaryLocation] = useState(DEFAULT_REGION);
  ...
  const submittedDateTime = `${formatDateTime(screenStartTime)} ${formatTime(
    screenStartTime
  )}`;
  ...
  const editMapHtml = useMemo(() => {
    if (!mapEditorSeed) return "";

    return createEditableMapHtml({
      latitude: mapEditorSeed.latitude,
      longitude: mapEditorSeed.longitude,
  ...
  const handleMapWebViewMessage = (event) => {
    try {
    ...
    const restorePendingImagePickerResult = async () => {
      try {
        ...
        if (
          ...
          addPickedPhotoAssets(pendingResult.assets);
        }
      } catch (error) {
  ...
  const updateAddressFromCoords = async (coords, shouldSetConfirmed = false) => {
    try {
      ...
      setLocationText(buildAddress(result?.[0]));
    } catch {
      setLocationText(
  ...
  const locateUser = async () => {
    try {
      ...
      if (mapModalVisible) {
        setTemporaryLocation(nextRegion);
        mapWebViewRef.current?.injectJavaScript(
          `window.flyToLocation(${nextRegion.longitude}, ${nextRegion.latitude}); true;`
        );
      } else {
  ...
  const handleSendMessage = () => {
    ...
    if (chatStep === 2) {
      cleanMessage = cleanMessage.replace(/\D/g, "").slice(0, 11);
    }

    if ((!cleanMessage && !voiceNoteUri) || isRecording) return;

    if (chatStep === 2 && !/^\d{11}$/.test(cleanMessage)) {
      Alert.alert(
  ...
  const editDescription = () => {
    setMessage(complaintDescription);
    setDescriptionMessage(null);
    setContactMessage(null);
    setComplaintDescription("");
    setContactNumber("");
  ...
  const confirmTemporaryLocation = async () => {
    ...
    setMapModalVisible(false);
    setMapEditorSeed(null);
    await updateAddressFromCoords(finalLocation, true);
  ...
  const addPickedPhotoAssets = (assets = []) => {
    ...
    const usableAssets = assets.filter((asset) => Boolean(asset?.uri));
    ...
    const validAssets = usableAssets
    ...
    setSelectedPhotos((prevPhotos) =>
      [...prevPhotos, ...validAssets].slice(0, MAX_PHOTOS)
    );
  ...
  const pickPhotos = async () => {
    ...
    try {
      ...
      addPickedPhotoAssets(result.assets);
    } catch (error) {
  ...
  const uploadComplaintPhotos = async (complaintId) => {
    ...
    for (let index = 0; index < selectedPhotos.length; index += 1) {
      ...
      const preparedPhoto = await normalizePhotoForUpload(photo);
  ...
  const handleSubmitComplaint = async () => {
    ...
    if (!/^\d{11}$/.test(cleanContactNumber)) {
      Alert.alert(
        "Invalid Contact Number",
        "Contact number must be exactly 11 digits."
      );
    ...
    try {
      ...
      try {
        ...
        Alert.alert(
          "Photo Upload Failed",
          "Your complaint was saved, but the photo upload failed. Please try updating the complaint later."
        );
      }
        ...
        [
          {
            ...
            onPress: () =>
              router.replace({
                pathname: "/(tabs)/citizenComplaints",
                params: {
  ...
  const renderLocationPreview = (compact = false) => {
    return (
      <TouchableOpacity
        activeOpacity={0.9}
        ...
        style={[
          styles.reviewMapBox,
          compact ? styles.reviewMapBoxCompact : styles.reviewMapBoxFull,
        ]}
      >
        <WebView
          originWhitelist={["*"]}
        ...
        <View style={styles.mapEditOverlay}>
          ...
          <Text style={styles.mapEditOverlayText}>Edit Location</Text>
        </View>
      </TouchableOpacity>
    );
  };
    ...
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      ...
      <View style={styles.mainContainer}>
        ...
        >
          ...
          {chatStep >= 1 && complaintType === "emergency" && (
            <>
              ...
              <TouchableOpacity activeOpacity={0.7} style={styles.hotlineCard}>
                <View style={styles.hotlineIconCircle}>
                  <Ionicons name="call" size={18} color={RED} />
                </View>

                <View style={styles.hotlineTextBox}>
                  <Text style={styles.hotlineTitle}>Call Emergency Hotline</Text>
                  <Text style={styles.hotlineSubtitle}>911</Text>
                </View>
              ...
              <TouchableOpacity activeOpacity={0.7} style={styles.hotlineCard}>
                <View style={styles.hotlineIconCircle}>
                  <FontAwesome5 name="ambulance" size={15} color={RED} />
                </View>

                <View style={styles.hotlineTextBox}>
                  <Text style={styles.hotlineTitle}>Call CDRRMO / Rescue</Text>
                  <Text style={styles.hotlineSubtitle}>09XX XXX XXX</Text>
                </View>
          ...
          {chatStep >= 3 &&
            renderBotMessage(
              "Please confirm your complaint location. Tap the map to move the pin if needed.",
              formatTime(new Date())
            )}
          ...
          {chatStep >= 3 && (
            <View style={styles.locationCard}>
              <View style={styles.locationCardHeader}>
                ...
                <Text style={styles.locationCardTitle}>Complaint Location</Text>
              </View>

              <Text style={styles.locationCardAddress}>
                {locationText || "Detecting your location..."}
              </Text>

              {renderLocationPreview(true)}

              <Text style={styles.locationCardCoordinates}>
                Lat {selectedLocation.latitude.toFixed(5)} • Long{" "}
            ...
            <View style={styles.reviewCard}>
              <View style={styles.reviewItem}>
                <View style={styles.reviewTextBox}>
                  <Text style={styles.reviewLabel}>Complaint Title</Text>
                  <Text style={styles.reviewValue}>{complaintTitle}</Text>
                </View>

                <TouchableOpacity activeOpacity={0.7} onPress={editTitle}>
              ...
              <View style={styles.reviewItem}>
                ...
                <TouchableOpacity activeOpacity={0.7} onPress={editDescription}>
                  <Feather name="edit-2" size={15} color={GREEN} />
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.reviewItem}>
                ...
                <TouchableOpacity activeOpacity={0.7} onPress={editContact}>
                  <Feather name="edit-2" size={15} color={GREEN} />
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.reviewLocationSection}>
                <View style={styles.reviewItem}>
                  <View style={styles.reviewTextBox}>
                    <Text style={styles.reviewLabel}>Exact Complaint Location</Text>
                    <Text style={styles.reviewValue}>{locationText}</Text>
                  </View>

                  <TouchableOpacity activeOpacity={0.7} onPress={openEditLocation}>
                    <Feather name="edit-2" size={15} color={GREEN} />
                  </TouchableOpacity>
                </View>
                ...
                <View style={styles.addressOnlyBox}>
                  <Ionicons name="location-sharp" size={18} color={RED} />
                  <Text style={styles.addressOnlyText}>
                    {locationText || "Pinned complaint location"}
                  </Text>
                </View>
        ...
        <Modal
          ...
          onRequestClose={closeEditLocation}
        >
          <SafeAreaView style={styles.modalSafeArea} edges={["top", "bottom"]}>
            <View style={styles.mapModalHeader}>
              ...
              <View style={styles.modalTitleBox}>
                ...
                <Text style={styles.modalSubtitle}>
                  Tap the map to update the exact complaint location
                </Text>
              </View>
            ...
            <View style={styles.modalMapWrapper}>
              {mapModalVisible && mapEditorSeed && editMapHtml ? (
                <WebView
                  ref={mapWebViewRef}
                  originWhitelist={["*"]}
                  source={{
                  ...
                  onLoadEnd={() => {
                    mapWebViewRef.current?.injectJavaScript(
                      "if (window.map) { window.map.resize(); } true;"
                    );
                  }}
              ...
              <View style={styles.modalAttributionBox}>
            ...
            <View style={styles.modalBottomSheet}>
              ...
              <View style={styles.coordinatesBox}>
                <Text style={styles.coordinatesLabel}>Selected Pin</Text>
                <Text style={styles.coordinatesText}>
                  Lat {temporaryLocation.latitude.toFixed(5)} • Long{" "}
                  {temporaryLocation.longitude.toFixed(5)}
                </Text>
              </View>
              ...
              <TouchableOpacity
                ...
                onPress={confirmTemporaryLocation}
              >
