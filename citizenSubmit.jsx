...
function getAssignedOffice(category, existingOffice = null) {
  if (existingOffice && existingOffice !== "Unassigned") return existingOffice;
...
async function normalizePhotoForUpload(photo) {
  if (!photo?.uri) throw new Error("Selected photo has no file URI.");
...
export default function CitizenSubmit() {
  ...
  const handleSafeBack = () => {
    if (router.canGoBack?.()) {
      ...
      router.replace("/(tabs)/citizenDashboard");
    }
  };
  ...
  const updateAddressFromCoords = async (coords, shouldSetConfirmed = false) => {
    ...
    if (shouldSetConfirmed) setConfirmedLocation(true);
  };
  ...
  const openEditLocation = () => {
    Keyboard.dismiss();
    setTemporaryLocation(selectedLocation);
    setMapModalVisible(true);

    setTimeout(() => {
      modalMapRef.current?.animateToRegion(selectedLocation, 350);
    }, 250);
  };
  ...
  const addPickedPhotoAssets = (assets = []) => {
    ...
    if (remainingSlots <= 0) {
      Alert.alert("Photo Limit Reached", "You can only upload up to 3 photos.");
      return;
    }
    ...
    if (usableAssets.length > remainingSlots) {
      Alert.alert(
        ...
        `Only ${remainingSlots} more photo${
          remainingSlots > 1 ? "s" : ""
        } can be added. The limit is 3 photos only.`
      );
    }
  ...
  const uploadComplaintPhotos = async (storageFolderName) => {
    const uploadedPhotoUrls = [];

    if (!storageFolderName) {
      throw new Error("Missing storage folder name.");
    }
    ...
    for (let index = 0; index < selectedPhotos.length; index += 1) {
      ...
      const filePath = `${storageFolderName}/photo-${
        index + 1
      }-${Date.now()}.jpg`;
  ...
  const handleSubmitComplaint = async () => {
    ...
    try {
      ...
      const defaultPriority =
        complaintType === "emergency" ? "Urgent" : "Normal";

      const storageFolderName = `${user.id}/${Date.now()}`;
      const photoUrls = await uploadComplaintPhotos(storageFolderName);
        ...
        [
          {
            text: "OK",
            onPress: () => {
              router.replace("/(tabs)/citizenComplaints");
            },
          },
        ]
      );
          ...
          <SafeAreaView style={styles.modalSafeArea}>
            <View style={styles.mapModalHeader}>
              ...
              <View style={styles.modalTitleBox}>
                ...
                <Text style={styles.modalSubtitle}>
                  Drag the pin or tap the map to update the location
                </Text>
              </View>
            ...
            <View style={styles.modalMapWrapper}>
              <MapView
                ...
                initialRegion={temporaryLocation}
                mapType="none"
                onPress={(event) => {
...
const styles = StyleSheet.create({
  ...
  assistantBotLogo: { width: 30, height: 30, resizeMode: "contain" },
  assistantTextBox: { flex: 1 },
  assistantTitle: {
    fontFamily: "Poppins_700Bold",
    fontSize: 15.8,
    color: TEXT,
  ...
  smallBotLogo: { width: 22, height: 22, resizeMode: "contain" },
  botBubble: {
  ...
  reviewItemNoEdit: { marginBottom: 14 },
  reviewTextBox: { flex: 1, paddingRight: 12 },
  reviewLabel: {
    fontFamily: "Poppins_700Bold",
    fontSize: 13,
    color: GREEN,
  ...
  reviewMap: { width: "100%", height: "100%" },
  inlineSubmitButton: {
    height: 54,
    borderRadius: 8,
    backgroundColor: GREEN,
  ...
  inlineSubmitButtonDisabled: { opacity: 0.7 },
  ...
  modalSafeArea: { flex: 1, backgroundColor: WHITE },
  mapModalHeader: {
  ...
  modalTitleBox: { flex: 1, marginTop: 4 },
  modalTitle: {
    fontFamily: "Poppins_700Bold",
    fontSize: 18,
    color: TEXT,
