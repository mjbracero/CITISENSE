...
import useModeratorUnreadNotifications from "./useModeratorUnreadNotifications";
...
const H_PADDING = 20;
const DEFAULT_MODERATOR_DEPARTMENT = "";
...
const bottomTabs = [
  ...
  },
  {
    ...
    route: "/moderatorAssignedComplaints",
    activePath: "moderatorAssignedComplaints",
    flex: 1.55,
  },
  {
    label: "Notifications",
    ...
    activePath: "moderatorNotification",
    flex: 1.15,
  },
  {
    label: "Profile",
    ...
    route: "/moderatorProfile",
    activePath: "moderatorProfile",
    flex: 0.82,
  },
];
          ...
          window.updateModeratorPosition = function(lat, lng, heading) {
            ...
            if (!moderatorMarker) {
              ...
              moderatorMarker = new maplibregl.Marker({ element: moderatorMarkerElement, anchor: "center" })
                ...
                .setPopup(new maplibregl.Popup().setText("Moderator Location"))
                .addTo(map);
            } else {
...
export default function ModeratorAssignedComplaints() {
  const router = useRouter();
  const pathname = usePathname();
  const { unreadNotificationCount } = useModeratorUnreadNotifications();

  const [complaints, setComplaints] = useState([]);
  const [loadingComplaints, setLoadingComplaints] = useState(true);
  const [moderatorDepartment, setModeratorDepartment] = useState(DEFAULT_MODERATOR_DEPARTMENT);
  ...
  const [moderatorLocation, setModeratorLocation] = useState(null);
  const [moderatorHeading, setModeratorHeading] = useState(0);
  ...
  const categoryFilters = useMemo(() => {
    const departmentCategories = getCategoriesForDepartment(moderatorDepartment);
  ...
  }, [complaints, moderatorDepartment]);
  ...
  const loadModeratorDepartment = useCallback(async () => {
    try {
      const {
        data: { user },
        error: userError,
      ...
      if (profileByEmailError) {
        console.log(
          "Load moderator profile by email error:",
          profileByEmailError
        );
      } else {
      ...
      setModeratorDepartment(nextDepartment);

      console.log("Moderator assigned complaints department resolved:", {
        email: userEmail,
        authUserId: user.id,
  ...
  const loadAssignedComplaints = useCallback(
    async (showLoader = true) => {
      try {
        ...
        const department = await loadModeratorDepartment();
    ...
    [loadCitizenProfiles, loadModeratorDepartment]
  );
  ...
  const startModeratorTracking = useCallback(async () => {
    try {
      ...
      if (permission.status !== "granted") {
        Alert.alert(
          ...
          "Please allow location permission to show moderator location on the map."
        );
        return;
      }
      ...
      if (
        ...
        setModeratorHeading(currentLocation.coords.heading);
      }
      ...
      locationWatcherRef.current = await Location.watchPositionAsync(
        ...
        (updatedLocation) => {
          ...
          if (
            ...
            setModeratorHeading(updatedLocation.coords.heading);
          }
        }
      );
      ...
      headingWatcherRef.current = await Location.watchHeadingAsync(
        (headingData) => {
          ...
          if (typeof nextHeading === "number" && nextHeading >= 0) {
            setModeratorHeading(nextHeading);
          }
        }
      );
    } catch {
      Alert.alert(
        "Location Error",
        "Unable to get moderator location. Please try again."
      );
    } finally {
  ...
  const filteredComplaints = useMemo(() => {
    const filtered = complaints.filter((item) => {
      const departmentMatch = item.department === moderatorDepartment;
  ...
  }, [complaints, moderatorDepartment, selectedCategory, selectedStatus, selectedPriority]);
  ...
  const injectModeratorMapUpdate = useCallback(
    (delay = 0) => {
      if (!moderatorLocation) return;

      const jsCode = `
        if (window.updateModeratorPosition) {
          window.updateModeratorPosition(
            ${moderatorLocation.latitude},
            ${moderatorLocation.longitude},
            ${moderatorHeading}
          );
        }
        true;
    ...
    [moderatorLocation, moderatorHeading]
  );

  useEffect(() => {
    injectModeratorMapUpdate(0);
  }, [injectModeratorMapUpdate, selectedComplaint, fullMapVisible]);
  ...
  const routeToPinnedLocation = () => {
    ...
    if (!moderatorLocation) {
      Alert.alert(
        "Moderator Location Needed",
        "Please update moderator location first before routing."
      );
      return;
    }
  ...
  const notifyCitizenAboutComplaintChange = useCallback(
    async ({ complaint, type, title, message, status, metadata = {} }) => {
      if (!complaint?.citizenId || !complaint?.rawId) {
        console.log("Notification skipped: missing citizenId or complaintId", {
      ...
      try {
        const { error } = await supabase.from("complaint_notifications").insert({
          citizen_id: complaint.citizenId,
          complaint_id: complaint.rawId,
          type,
          title,
          message,
          status,
        ...
        console.log("Create citizen notification catch error:", error);
        return false;
      }
    },
    []
  ...
  const openUpdateStatusModal = (complaint) => {
    if (complaint.status === "Completed") {
      Alert.alert(
        "Status Locked",
        "Completed complaints can no longer be updated by the moderator. The admin is responsible for marking complaints as completed after reviewing citizen feedback."
      );
      return;
    }
  ...
  const selectStatus = async (nextStatus) => {
    ...
    if (actionComplaint.status === "Completed") {
      Alert.alert(
        "Status Locked",
        "Completed complaints can no longer be updated by the moderator."
      );
      setStatusModalVisible(false);
      setActionComplaint(null);
      return;
    ...
    try {
      ...
      if (!updatedComplaint) {
        Alert.alert(
          "Update Failed",
          "No complaint row was updated. Please check your moderator update policy."
        );
        return;
      }
      ...
      const notificationCreated = await notifyCitizenAboutComplaintChange({
      ...
      Alert.alert(
        "Status Updated",
        notificationCreated
          ? `Complaint status changed to ${nextStatus}. The citizen has been notified.`
          : `Complaint status changed to ${nextStatus}, but the citizen notification was not created. Check your complaint_notifications insert policy.`
      );
    ...
    try {
      ...
      setComplaints((prev) =>
        prev
          ...
          .filter((item) => item.department === moderatorDepartment)
      );

      if (selectedComplaint?.rawId === actionComplaint.rawId) {
        if (nextDepartment === moderatorDepartment) {
          setSelectedComplaint((prev) =>
            prev
              ? {
                  ...prev,
        ...
        >
          <View style={styles.headerContainer}>
            ...
            <Text style={styles.headerSubtitle}>
              {moderatorDepartment || "No department assigned in database"}
            </Text>
          </View>
        ...
        <View style={styles.bottomNav}>
          {bottomTabs.map((tab) => {
            const isActive =
              ...
              (tab.label === "Home" &&
                (pathname === "/" || pathname?.includes("moderatorDashboard")));
                  ...
                  <View style={styles.locationSection}>
                    ...
                    <View style={styles.mapBox}>
                      <WebView
                        ...
                        domStorageEnabled
                        scrollEnabled={false}
                        onLoadEnd={() => injectModeratorMapUpdate(350)}
                    ...
                    <TouchableOpacity
                      ...
                      onPress={startModeratorTracking}
                    >
                      {locationLoading ? (
            ...
            {fullMapVisible && selectedComplaint && (
              <View style={styles.fullMapOverlay}>
                ...
                <WebView
                  ...
                  javaScriptEnabled
                  domStorageEnabled
                  onLoadEnd={() => injectModeratorMapUpdate(450)}
