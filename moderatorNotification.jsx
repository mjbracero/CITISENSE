...
export default function ModeratorNotification() {
  ...
  } = useModeratorUnreadNotifications();
  ...
  const loadNotifications = useCallback(async (showLoader = true) => {
    try {
      ...
      if (error) {
        console.log("Load moderator notifications error:", error);
        setNotifications([]);
        return;
      }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`moderator-notifications-ui-${currentModeratorId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
          ...
          filter: `moderator_id=eq.${currentModeratorId}`,
        },
        () => {
          loadNotifications(false);
          reloadUnreadNotificationCount?.();
      ...
      .subscribe((status) => {
        console.log("Moderator notifications realtime status:", status);
      });

    return () => {
      supabase.removeChannel(channel);
    };
  }, [currentModeratorId, loadNotifications, reloadUnreadNotificationCount]);
  ...
  const markSingleAsRead = async (notification) => {
    if (!notification?.id || !currentModeratorId || !notification.unread) return;
    ...
    const { error } = await supabase
      ...
      .eq("moderator_id", currentModeratorId);

    if (error) {
      console.log("Mark moderator notification read error:", error);
      loadNotifications(false);
      return;
    }
  ...
  const markAllAsRead = async () => {
    if (!currentModeratorId || unreadNotificationCount <= 0) return;
  ...
  const openRelatedComplaint = async (notification = selectedNotification) => {
    if (!notification?.complaintId) {
      ...
      router.replace("/moderatorAssignedComplaints");
      return;
    }
    ...
    router.push({
      pathname: "/moderatorAssignedComplaints",
      params: {
